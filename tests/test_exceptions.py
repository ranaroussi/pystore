...
class TestExceptions:
    ...
    def test_collection_not_found_error(self, test_store):
        ...
        assert "Collection 'nonexistent_collection' does not exist" in str(
            exc_info.value
        )

    def test_collection_exists_error(self, test_store):
        """Test existing collections can be reopened without overwrite"""
        collection1 = test_store.collection("existing_collection")
        collection2 = test_store.collection("existing_collection", overwrite=False)
    ...
    def test_data_integrity_error(self, test_collection):
        ...
        with pytest.raises(pystore.DataIntegrityError) as exc_info:
            test_collection.append(
                "test_item", duplicate_data, duplicate_handling="error"
            )
