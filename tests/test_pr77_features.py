...
class TestMultiIndexSupport:
    ...
    def test_complex_multiindex(self):
        ...
        df = pd.DataFrame(
            {"value1": np.random.randn(8), "value2": np.random.randint(0, 100, 8)},
            index=index,
        )
...
class TestComplexDataTypes:
        ...
        df = pd.DataFrame(
            {
                "category": pd.Categorical(
                    ["A", "B", "A", "C"], categories=["A", "B", "C", "D"], ordered=True
                ),
                "value": [1, 2, 3, 4],
            }
        )
    ...
    def test_interval_support(self):
        ...
        df = pd.DataFrame(
            {"interval": pd.interval_range(start=0, end=4), "value": [1, 2, 3, 4]}
        )
...
class TestTimezoneSupport:
    ...
    def test_timezone_preservation(self):
        ...
        # Verify timezone is preserved while storage stays UTC internally
...
class TestTransactionSupport:
    ...
    def test_successful_transaction(self):
        ...
        pd.testing.assert_frame_equal(self.collection.item("item1").to_pandas(), df1)
        pd.testing.assert_frame_equal(self.collection.item("item2").to_pandas(), df2)
    ...
    def test_batch_transaction(self):
        ...
        data = {
            f"item_{i}": pd.DataFrame({"value": [i, i + 1, i + 2]}) for i in range(10)
        }
...
class TestAsyncOperations:
    ...
    async def test_async_batch_write(self):
        ...
        assert len(items) == 3
        for name, df in data.items():
            pd.testing.assert_frame_equal(self.collection.item(name).to_pandas(), df)
...
class TestSchemaEvolution:
    ...
    def test_add_only_strategy(self):
        ...
        # Initial schema
        df_v1 = pd.DataFrame(
            {"value": [1, 2, 3], "timestamp": pd.date_range("2023-01-01", periods=3)}
        )
    ...
    def test_schema_migration(self):
        ...
        # Initial data
        df_v1 = pd.DataFrame({"value": [1, 2, 3], "old_name": ["a", "b", "c"]})
        ...
        def data_generator():
            for i in range(10):
                df = pd.DataFrame(
                    ...
                    index=pd.date_range(
                        f"2023-01-{i + 1:02d}", periods=1000, freq="min"
                    ),
                )
                yield df
...
class TestEdgeCases:
    ...
    def test_duplicate_index_handling(self):
        ...
        df = pd.DataFrame({"value": [1, 2, 3, 4]}, index=[1, 1, 2, 2])
