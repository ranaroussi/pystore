...
def sample_data_with_duplicates():
    ...
    # Repeat some dates
    duplicate_dates = dates.append(dates[40:])
    data = pd.DataFrame(
        {
...
def sample_data_nanosecond():
    ...
    dates_with_ns = dates + pd.to_timedelta(
        np.random.randint(0, 1000, size=10), unit="ns"
    )
    data = pd.DataFrame({"value": np.random.randn(10)}, index=dates_with_ns)
    return data
